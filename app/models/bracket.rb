...
class Bracket < ApplicationRecord
  ...
  def possible_points
    ...
    if !$scorecard.pick_third && Team.exists?(:name => self.pick_third) && !Team.find_by(:name => self.pick_third).eliminated || (Team.exists?(:name => self.pick_third) && Team.find_by(:name => self.pick_third).eliminated && (self.pick_third == $scorecard.pick_s1 || self.pick_third == $scorecard.pick_s2 || self.pick_third == $scorecard.pick_s3 || self.pick_third == $scorecard.pick_s4))
      possible_counter += 20
    end
